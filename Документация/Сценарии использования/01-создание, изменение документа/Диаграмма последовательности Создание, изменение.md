...
sequenceDiagram
    ...
    Note over Main, Editor: 1) СОЗДАНИЕ НОВОГО ДОКУМЕНТА
    ...
    Editor->>Document: SplitIfNeeded()  %% проверка структуры после создания
    Editor-->>Main: Document готов
    deactivate Editor

    Note over Editor, CompositeManager: 2) ВВОД ТЕКСТА ("Hello world\\nHi")

    Main->>Editor: пользователь вводит текст
    activate Editor
    ...
    Note over Editor, CompositeManager: 3) ИЗМЕНЕНИЕ ДОКУМЕНТА (ReplaceAll)

    Main->>Editor: ReplaceAll("world", "planet")
    activate Editor
