...
sequenceDiagram
    ...
    Note over User, Editor: 1) СОХРАНЕНИЕ ДОКУМЕНТА (в DOCX)

    User->>Main: SaveDocument("mytext.docx")
    ...
    Note over User, Editor: 2) ЭКСПОРТ В PDF

    User->>Main: ExportDocument("pdf", "mytext.pdf")
    ...
    Note over User, Editor: 3) ЭКСПОРТ В JSON

    User->>Main: ExportDocument("json", "mytext.json")
